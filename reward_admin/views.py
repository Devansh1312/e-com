...
class UserProfileView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        context = {
            "user": user,
            "breadcrumb": {"parent": "Account", "child": "Profile"},
        }
...
class CountryListView(View):
    def get(self, request):
        ...
        return render(request, 'Admin/Location/Country_List.html', {
            'countries': countries,
            'breadcrumb': {'parent': 'Location Management', 'child': 'Country Management'}
        })
...
class StateListView(View):
    def get(self, request):
        ...
        return render(request, 'Admin/Location/State_List.html', {
            'states': states,
            'countries': countries,
            'breadcrumb': {'parent': 'Location Management', 'child': 'State Management'}
        })
...
class CityListView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return render(
            ...
            self.template_name,
            {
                "breadcrumb": {"parent": "Location Management", "child": "City Management"},
                "states": states,
            },
        )
...
class CustomerUserListView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        return render(
            ...
            self.template_name,
            {
                "breadcrumb": {"parent": "User Management", "child": "Customer List"},
            },
        )
    ...
    def post(self, request):
        ...
        # Base queryset - OPTIMIZED with select_related and prefetch_related
        queryset = User.objects.filter(role_id=2).select_related('role').prefetch_related('cities')
        ...
        # Get total records count (without filters) - OPTIMIZED
        ...
        for index, customer in enumerate(customers):
            ...
            # Get assigned cities - OPTIMIZED (uses prefetch_related data)
            cities_list = list(customer.cities.all())
            cities = ", ".join([city.name for city in cities_list[:2]])
            if len(cities_list) > 2:
                cities += f" (+{len(cities_list) - 2} more)"
...
class CustomerDetailView(LoginRequiredMixin, View):
    ...
    def get(self, request, pk):
        try:
            ...
            customer_wishlist = wishlist.objects.filter(customer=customer).select_related(
                'product', 'product__category'
            ).prefetch_related('product__images').order_by('-created_at')
            
            # Get customer's reviews
            customer_reviews = customer_review.objects.filter(customer=customer).select_related(
                'product', 'product__category'
            ).order_by('-created_at')
            ...
            return render(
                ...
                self.template_name,
                {
                    ...
                    "breadcrumb": {
                        "grandparent": "User Management",
                        "parent": "Customer List",
                        "parent_url": reverse('customer_user_list'),
                        "child": f"Customer Detail - {customer.name or customer.username}"
                    },
                },
            )
        except User.DoesNotExist:
...
class FAQView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return render(
            ...
            {
                "faqs": faqs,
                "breadcrumb": {"parent": "General Settings", "child": "FAQ Management"},
            },
        )
...
class SizeListView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return render(
            ...
            {
                "sizes": sizes,
                "breadcrumb": {"parent": "General Settings", "child": "Size Management"},
            },
        )
...
class ColorListView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return render(
            ...
            {
                "colors": colors,
                "breadcrumb": {"parent": "General Settings", "child": "Color Management"},
            },
        )
...
class ProductCategoryListView(View):
    def get(self, request):
        ...
        return render(request, 'Admin/Product/Category_List.html', {
            ...
            'breadcrumb': {'parent': 'Product Management', 'child': 'Category Management'}
        })
...
class ProductListView(View):
    def get(self, request):
        # OPTIMIZED: Use Prefetch objects for better control
        images_prefetch = Prefetch('images', queryset=product_image.objects.all())
        variants_prefetch = Prefetch('variants', queryset=product_variant.objects.select_related('size', 'color'))
        
        products = (
            product.objects.all()
            .select_related('category')
            .prefetch_related(images_prefetch, variants_prefetch)
            .order_by('-created_at')
        )
        ...
        return render(request, 'Admin/Product/Product_List.html', {
            ...
            'sizes': sizes,
            'colors': colors,
            'breadcrumb': {'parent': 'Product Management', 'child': 'Product List'}
        })
...
class CustomerReviewListView(View):
    def get(self, request):
        ...
        return render(request, 'Admin/Review/Review_List.html', {
            'reviews': reviews,
            'breadcrumb': {'parent': 'Review Management', 'child': 'Customer Reviews'}
        })
...
class ContactUsListView(View):
    def get(self, request):
        ...
        return render(request, 'Admin/Contact/Contact_List.html', {
            'contacts': contacts,
            'breadcrumb': {'parent': 'Contact Management', 'child': 'Contact Messages'}
        })
...
class DashboardBannerListView(View):
    def get(self, request):
        ...
        return render(request, 'Admin/Dashboard/Banner_List.html', {
            'banners': banners,
            'breadcrumb': {'parent': 'Dashboard Management', 'child': 'Banner Management'}
        })
